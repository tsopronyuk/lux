...
static bool SignStep(const BaseSignatureCreator& creator, const CScript& scriptPubKey,
                     std::vector<valtype>& ret, txnouttype& whichTypeRet, SigVersion sigversion, bool fCoinStake = false)
{
    CScript scriptRet;
    uint160 h160;
    ret.clear();
    ...
    if (HasColdstakeOp(scriptPubKey)) {
        ...
        if (creator.IsCoinStake()) {
            if (!GetColdstakeScriptPath(scriptPubKey, scriptPath))
                return false;
        } else {
            if (!GetNonColdstakeScriptPath(scriptPubKey, scriptPath))
                return false;
        }
...
bool ProduceSignature(const BaseSignatureCreator& creator, const CScript& fromPubKey, SignatureData& sigdata, bool fCoinStake)
{
    ...
    bool solved = SignStep(creator, script, result, whichType, SIGVERSION_BASE, fCoinStake);
    bool P2SH = false;
    CScript subscript;
    ...
    if (solved && whichType == TX_SCRIPTHASH)
    {
        ...
        solved = SignStep(creator, script, result, whichType, SIGVERSION_BASE) && whichType != TX_SCRIPTHASH;
        P2SH = true;
    }
    ...
    else if (solved && whichType == TX_WITNESS_V0_SCRIPTHASH)
    {
        ...
        txnouttype subType;
        solved = SignStep(creator, witnessscript, result, subType, SIGVERSION_WITNESS_V0) && subType != TX_SCRIPTHASH && subType != TX_WITNESS_V0_SCRIPTHASH && subType != TX_WITNESS_V0_KEYHASH;
