...
void PeerLogicValidation::NewValidBlock(const CBlockIndex *pindex, const std::shared_ptr<const CBlock>& pblock) {
    ...
    CBlockIndex* pindexPrev = LookupBlockIndex(pblockk->hashPrevBlock);
    bool usePhi2 = pindexPrev ? pindexPrev->nHeight >= Params().SwitchPhi2Block() : false;
    uint256 hashBlock(pblock->GetHash(usePhi2));
...
void PeerLogicValidation::BlockChecked(const CBlock& block, const CValidationState& state) {
    ...
    const uint256 hash(block.GetHash(usePhi2));
...
void static ProcessGetBlockData(CNode* pfrom, const Consensus::Params& consensusParams, const CInv& inv, CConnman* connman, const std::atomic<bool>& interruptMsgProc)
{
    ...
    if (send && (mi->second->nStatus & BLOCK_HAVE_DATA))
    {
        ...
        CBlockIndex* pindexPrev = LookupBlockIndex(a_recent_block->hashPrevBlock);
        bool usePhi2 = pindexPrev ? pindexPrev->nHeight >= Params().SwitchPhi2Block() : false;
        if (a_recent_block && a_recent_block->GetHash(usePhi2) == (*mi).second->GetBlockHash()) {
...
bool static ProcessMessage(CNode* pfrom, const std::string& strCommand, CDataStream& vRecv, int64_t nTimeReceived, const CChainParams& chainparams, CConnman* connman, const std::atomic<bool>& interruptMsgProc)
{
    ...
    if (pfrom->cleanSubVer != "" && GetMajorVersionFromVersion(pfrom->cleanSubVer) < CLIENT_VERSION_MAJOR && chainActive.Height() >= chainparams.SwitchPhi2Block()) {
        connman->PushMessage(pfrom, CNetMsgMaker(INIT_PROTO_VERSION).Make(NetMsgType::REJECT), strCommand, REJECT_OBSOLETE, std::string("Peer is obsolete"));
        pfrom->fDisconnect = true;
        return false;
    }
...
#       endif
    ...
    else if (strCommand == NetMsgType::CMPCTBLOCK && !fImporting && !fReindex) // Ignore blocks received while importing
    {
        ...
        if (fBlockReconstructed) {
            ...
            {
                LOCK(cs_main);
                mapBlockSource.emplace(pblock->GetHash(usePhi2), std::make_pair(pfrom->GetId(), false));
            }
            ...
            if (fNewBlock) {
                ...
                LOCK(cs_main);
                mapBlockSource.erase(pblock->GetHash(usePhi2));
            }
    ...
    else if (strCommand == NetMsgType::BLOCK && !fImporting && !fReindex) // Ignore blocks received while importing
    {
        ...
        LogPrint(BCLog::NET, "received block %s peer=%d\n", pblock->GetHash(usePhi2).ToString(), pfrom->GetId());

        bool forceProcessing = false;
        const uint256 hash(pblock->GetHash(usePhi2));
        {
            LOCK(cs_main);
